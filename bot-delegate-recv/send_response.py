#!/usr/bin/env python3
"""
send_response.py — 向群聊发送带 @ 的富文本回执消息

用法:
  python3 send_response.py <response_message> <chat_id> <target_open_id>

参数:
  response_message  —— 回执内容
  chat_id           —— 群聊 ID (oc_xxx)
  target_open_id    —— 要 @ 的目标 open_id (ou_xxx)
...
TARGET_OPEN_ID = sys.argv[3] if len(sys.argv) > 3 else None

def die(msg):
    print(msg, file=sys.stderr)
    sys.exit(1)

if not MESSAGE or not CHAT_ID or not TARGET_OPEN_ID:
    die('用法: python3 send_response.py <response_message> <chat_id> <target_open_id>')
if not APP_ID:
...
def main():
    ...
    if status != 200 or msg_res.get('code') != 0:
        die(f'发送回执失败: {json.dumps(msg_res)}')
